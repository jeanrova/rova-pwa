...
const toIso = s => {                           // -> yyyy-mm-dd (desktop stores/matches ISO strings)
  const d = parseDmy(s);
  return d ? `${d.getFullYear()}-${pad2(d.getMonth()+1)}-${pad2(d.getDate())}` : (s || "").trim();
};
  ...
  dlg.addEventListener("close", async function h(){
    ...
    if (dlg.returnValue === "ok" && F.title.value.trim()){
      const rec = { id: e?.id ?? uuid(), title: F.title.value.trim(),
        date: toIso(F.date.value.trim()), end_date: toIso(F.end_date.value.trim()),
...
$("#ch-export").onclick = async () => {
  ...
  try {
    ...
    // Share sheet keeps the .rova name; the download fallback uses .zip because
    // iOS Safari silently rewrites unknown extensions (.rova -> .txr). A .rova
    // IS a zip, and the desktop import reads it regardless of extension.
    const shareName = "rova_export_" + stamp + ".rova";
    if (navigator.share && navigator.canShare?.({ files: [new File([blob], shareName)] })) {
      try {
        await navigator.share({ files: [new File([blob], shareName)], title: shareName });
        return;
      } catch (_) { /* user cancelled or share failed -> fall through to download */ }
    }
    const a = el("a");
    a.href = URL.createObjectURL(blob);
    a.download = "rova_export_" + stamp + ".zip";
